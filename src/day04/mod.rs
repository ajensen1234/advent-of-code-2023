...
impl ScratchCard {
    ...
    pub fn read_data(&mut self, input: &str) {
        ...
        let _num: i32 = card_num.parse::<i32>().unwrap();
    ...
    fn calculate_total_points(winning_numbers: &[i32], hand_numbers: &[i32]) -> u32 {
        ...
        for num in hand_numbers.iter() {
            if winning_numbers.contains(num) && !checked_nums.contains(num) {
